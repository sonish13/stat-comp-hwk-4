...
ggplot(df, aes(x, jitter(y, amount = 0.03))) +
  theme_minimal() +
  geom_jitter() +
  geom_ribbon(aes(ymin = pnorm(p,mean = m, sd=s),
                  ymax = pnorm(p,mean = m, sd=s),
...
# dnorm(0.997833779,mean = mean(df$p), sd=sd(df$p))
# pnorm(0.997833779,mean = mean(df$p), sd=sd(df$p))
